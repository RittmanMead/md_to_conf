...
class ConfluenceApiClient():
    ...
    def __init__(self, confluence_api_url, username, api_key, space_key, editor_version):
    ...
    def update_page(self, page_id, title, body, version, parent_id):
        ...
        response = self.check_errors_and_get_json(session.put(url, data=json.dumps(page_json)))
        ...
        if response["status_code"] == 200:
            ...
            link = '%s%s' % (self.confluence_api_url, data[u'_links'][u'webui'])
    ...
    def get_space_id(self):
        ...
        if response["status_code"] == 404:
            ...
            if len(data[u'results']) >= 1:
                self.space_id = data[u'results'][0][u'id']
    ...
    def create_page(self, title, body, parent_id):
        ...
        response = self.check_errors_and_get_json(self.get_session().post(url, data=json.dumps(new_page)))

        if response["status_code"] == 200:
            data = response["data"]
            space_id = data[u'spaceId']
            page_id = data[u'id']
    ...
    def get_page(self, title):
        ...
        url = '%s/api/v2/spaces/%s/pages?title=%s' % (
            self.confluence_api_url, space_id, urllib.parse.quote_plus(title))
    ...
    def get_attachment(self, page_id, filename):
        ...
        """
        url = '%s/api/v2/pages/%s/attachments?filename=%s' % (self.confluence_api_url, page_id, filename)
    ...
    def upload_attachment(self, page_id, file, comment):
        ...
        if attachment:
            url = '%s/rest/api/content/%s/child/attachment/%s/data' % (self.confluence_api_url, page_id, attachment.id)
        else:
            url = '%s/rest/api/content/%s/child/attachment/' % (self.confluence_api_url, page_id)
